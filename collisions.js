import { ball, cH, cW, $, player1, player2, ballInit, endGame } from "./index";

/* COLLISIONS */
/* Ball */
export function ballCollisions(ballOut) {
  if (ball.position.y + ball.size.height + ball.velocity.y >= cH ||
    ball.position.y + ball.velocity.y <= 0) {
    ball.velocity.y *= -1;
  }
  if (ball.position.x + ball.size.width + ball.velocity.x >= cW ||
...
export function playersCollisions() {
  ...
  /* Player1 */
  /* Corners */
  if (ball.position.x + ball.velocity.x <=
  ...
  /* Front and Back */
  if ((ball.position.x + ball.size.width + ball.velocity.x >=
    player1.position.x &&
  ...
  /* Player2 */
  /* Corners */
  if (ball.position.x + ball.size.width + ball.velocity.x >= player2.position.x &&
